...
fn solve(input: &str) -> Result<String, Error> {
    ...
    let mut qu = Vec::with_capacity(q);
    for _ in 0..q {
        let a = parse!(iter);
        qu.push(a);
    }
    ...
    i.into_iter().for_each(|e| {
        if ! seen.contains(&e) {
            ans.push(e);
        }
    });

    ans.extend(changed.into_iter().rev());

    let mut ans = ans.into_iter().map(|v| v.to_string()).collect::<Vec<String>>().join(" ");
    ans.push('\n');
