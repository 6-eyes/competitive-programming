...
fn solve(input: &str) -> Result<u8, Error> {
    ...
    let n = parse!(iter);
    let chairs = iter.next().ok_or(Error::Iter)?.chars().collect::<Vec<char>>();
    let mut ans = 0;
    for i in 0..n {
        if chairs[i] == 'x' && i.checked_sub(1).map(|v| chairs[v]).is_none_or(|c| c == 'x') && chairs.get(i + 1).is_none_or(|v| *v == 'x') {
            ans += 1;
        }
    }
